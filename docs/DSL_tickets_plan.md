...
- **DoD / tests (fixtures in `testdata/saves/the-bibites/`):** enumerate `save.bibites`; read attributes match normalized values; arbitrary gene read; missing attr → clean error. No mutation, no DuckDB yet (iteration is in-memory).
