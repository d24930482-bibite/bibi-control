...
> tested in `savemutator` — these tickets are binding-layer work.

### T10 — entity mutation validation guards (stub)
- **Status:** Not started. Split out of T6.
...
- **Files:** `script/thebibites/guards.go` — `Rule{ReadOnly, Min/Max, Enum, Type}` checked before `StageSQLSet`, **seeded from generated metadata** (the `Type` rule from the column `value_type`/`SQLType`; read-only already from `attrSpec.writable`) plus a small hand-maintained semantic `overrides` map for constraints metadata can't express (`energy`/`health` ≥ 0, `dead`/`dying` bool-only, `species_id` referential). Wire into `Entity.SetField` and the bulk `where(...).set` path (validate once per column). Per `sqlref-generation-philosophy`: derive what's derivable; keep `overrides` tiny.
- **Deps:** T6. **New module dep:** none.
- **DoD / tests:** guard rejects out-of-range / bad-enum / wrong-type with a clear diagnostic; bulk validates once per column; the derived type rule needs no per-column allowlist; overrides cover only the few semantic constraints. (T7 settings writes reuse this.)
...
### T11a — structural: whole-entity delete (stub)
- **Status:** Not started. Split out of T11.
- **Goal:** stage a whole-entity (`bibite`/`egg`) delete, honoring the consistency contract — staged for commit, **not** mirrored mid-run.
- **Files:** `script/thebibites/entity.go` / `save_value.go` — `b.delete()` building the identity ref via the existing `entityLocatorRef` (`loadedsave.go:254`) → `Session.StageSQLDelete` (entry path → `DeleteEntry`) or `DeleteEntryWithOptions` for prune control. `mirror.go` — mark the buffer **"structurally deferred"** for the op instead of emitting an `UPDATE`. No new binding category required.
- **Deps:** T6. **New module dep:** none.
- **DoD / tests:** whole-entity delete staged → reparse after commit shows the entry gone; referential guard refuses orphaning deletes (prune opt-in via `DeleteOptions.PruneParentLinks`); **a structural change is *not* visible to an in-run query but *is* present after commit.**

### T11b — structural: array element append/delete (stub)
- **Status:** Not started. Split out of T11.
- **Goal:** stage array-element append/delete (pellets / synapses / stomach contents), staged for commit and **not** mirrored mid-run.
- **Prerequisite:** the 1:many sub-collection binding — a new `attrCategory` beyond `categoryScalar` (`attr_registry.go:14-20`) exposing repeated sub-structures as iterable collections. *Optional finer split:* if this read surface proves large, peel it into its own ticket (a T4-shaped read addition) ahead of the writes; left as a seam, not pre-split.
- **Files:** `script/thebibites/attr_registry.go` (new sub-collection category), `collection.go` / a sub-collection value (`.append`/`.delete`), `mirror.go` (reuse T11a's structurally-deferred marker). Array `.append`/`.delete` built from `duckdb.ScanSQLRefs` (`SQLRefOpAppend`/`SQLRefOpDelete`) → `StageSQLAppend`/`StageSQLDelete`. Entry-level *append* (a whole bibite/egg) needs a cross-save workspace and stays out of scope (single-save `SQLAppend` rejects it).
- **Deps:** T6; reuses T11a's structurally-deferred marker. **New module dep:** none.
- **DoD / tests:** array append/delete reconciles scene counts (`SceneCount`); **a structural change is *not* visible to an in-run query but *is* present after commit.**
