...
> **Expanded mutator path (split out of T6 on 2026-06-16).** T6 shipped scalar
> set only; the two stubs below carry the rest of the entity-mutation surface so
> T6 stays small. Both build on T6's `SetField`/bulk-set/mirror plumbing.
...
### T11 — structural mutations: entity delete + array append/delete (stub)
- **Status:** Not started. Split out of T6.
- **Goal:** stage structural changes — whole-entity delete and array-element append/delete (pellets / synapses / stomach contents) — honoring the consistency contract: structural ops are staged for commit and are **not** mirrored mid-run.
- **Files:** `script/thebibites/save_value.go` / `entity.go` — `.delete()` (whole-entity → `Session.StageSQLDelete` → `DeleteEntry`) and array `.append`/`.delete` built from `duckdb.ScanSQLRefs` (`Op: append`/`delete`) → `StageSQLAppend`/`StageSQLDelete`. `mirror.go` — mark the buffer **"structurally deferred"** for these ops instead of emitting an `UPDATE`. Entry-level *append* (a whole bibite/egg) needs a cross-save workspace and stays out of scope (single-save `SQLAppend` rejects it).
- **Deps:** T6. **New module dep:** none.
- **DoD / tests:** whole-entity delete staged → reparse after commit shows the entry gone; referential guard refuses orphaning deletes (prune opt-in via `DeleteOptions.PruneParentLinks`); array append/delete reconciles scene counts (`SceneCount`); **a structural change is *not* visible to an in-run query but *is* present after commit.**
